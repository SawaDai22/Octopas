...
def danger():
    ...
    global fp
    global death
    diedThread = threading.Thread(target=you_died,daemon=False)
    if hp < hpMax:
        f=random.randint(0, 100)
...
def update():
    ...
    canvas.delete("id2")
    x = 200*(hp/hpMax)
    canvas.create_rectangle(50, 5, x+50, 10,fill="blue",tag="id2")
...
if __name__ == "__main__":
    ...
    canvas = tkinter.Canvas(root, width = 800, height = 100)
    canvas.create_rectangle(50, 5, 250, 10,fill="black",tag="id1")
    canvas.create_rectangle(50, 5, 200*(hp/hpMax)+50, 10,fill="blue",tag="id2")
    ...
    threadGrow = threading.Thread(target=grow,daemon=True)
    threadGrow.start()
    root.mainloop()
